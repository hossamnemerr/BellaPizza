...
class _GradientButtonState extends State<GradientButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _s = Tween<double>(begin: 1.0, end: 0.95).animate(
    ...
    return ScaleTransition(scale: _s,
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 200), height: widget.height,
          ...
          child: ClipRRect(borderRadius: BorderRadius.circular(r),
            child: Material(color: Colors.transparent,
              child: InkWell(onTap: _tap,
                splashColor: AppColors.onPrimary.withValues(alpha: 0.08),
                highlightColor: AppColors.onPrimary.withValues(alpha: 0.04),
  ...
  Widget _content() {
    ...
    final label = Text(widget.label, style: GoogleFonts.oswald(
        fontSize: 16, fontWeight: FontWeight.w600, color: color, letterSpacing: 1.0));
    final ic = widget.icon != null
    ...
    if (widget.iconPosition == IconPosition.leading) {
      return Row(mainAxisAlignment: MainAxisAlignment.center,
          children: [ic, const SizedBox(width: 10), label]);
    }
    return Row(mainAxisAlignment: MainAxisAlignment.center,
        children: [label, const SizedBox(width: 10), ic]);
  }
}
...
class PaymentButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GradientButton(
      label: 'إتمام الدفع', onPressed: onPressed, isLoading: isLoading,
...
class AddToOrderButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => GradientButton(
      label: 'أضف للطلب  $price', onPressed: onPressed, isLoading: isLoading,
