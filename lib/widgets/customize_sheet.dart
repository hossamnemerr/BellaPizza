// Bottom sheet showing customization options before adding an item to cart
...
// Show the customize item bottom sheet
...
class _CustomizeSheetState extends State<CustomizeSheet>
  ...
  Future<void> _bump() async {
    await _qtyCtrl.forward();
    await _qtyCtrl.reverse();
  }

  double get _total {
    double base = widget.prefillItem?.unitPrice ?? 0;
    double extras =
        _toppings.where((t) => t.on).fold(0, (sum, t) => sum + t.price);
  ...
  void _addToOrder() {
    ...
    CartProvider.read(context).addItem(CartItem(
      id: widget.prefillItem?.id ??
          'custom_${DateTime.now().millisecondsSinceEpoch}',
      name: widget.prefillItem?.name ?? 'عنصر مخصص',
      category: widget.prefillItem?.category ?? 'أخرى',
          ...
          child:
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                  child: Column(
                      ...
                      children: [
                    Text(widget.prefillItem?.name ?? 'Item',
                        style: GoogleFonts.oswald(
                            fontSize: 20,
                    ...
                    Text(widget.prefillItem?.description ?? '',
                    ...
                    .map((t) => GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            child:
                              ...
                              Icon(
                                  t.on ? Icons.check_circle : Icons.add_rounded,
                                  size: 16,
                                  color: t.on
                                      ? AppColors.onPrimary
                                      : Colors.white),
                              ...
                              if (t.price > 0) ...[
                                ...
                                Text('+${t.price.toStringAsFixed(0)} ج',
                                    style: GoogleFonts.oswald(
                                        fontSize: 11,
                                        fontWeight: FontWeight.w800,
                                        color: t.on
            ...
            ..._dietary.asMap().entries.map((e) => Padding(
                  padding: EdgeInsets.only(
                      bottom: e.key < _dietary.length - 1 ? 10 : 0),
                  child: GestureDetector(
                    onTap: () {
                      HapticFeedback.selectionClick();
                      setState(() => e.value.on = !e.value.on);
                    },
                    child: Container(
                        padding: const EdgeInsets.symmetric(
...
class _QtyBtnState extends State<_QtyBtn> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _s = Tween<double>(begin: 1.0, end: 0.78)
              ...
              child: Row(children: [
                Text('إضافة للطلب',
                    style: GoogleFonts.oswald(
                        ...
                        letterSpacing: -0.2)),
                const Spacer(),
                Text('${widget.price.toStringAsFixed(0)} ج',
                    style: GoogleFonts.oswald(
                        fontSize: 16,
                        fontWeight: FontWeight.w800,
                        color: AppColors.onPrimary,
